...
from PINN_DEIM.Functions.data_set_preparation import AllenCahnEquationDomainDecomposition, AllenCahnEquationEntireDomain
from PINN_DEIM.Functions.deep_simulation_framework import PDEDeepModelSplitDomain, AllenCahnDeepModelEntireDomain
from PINN_DEIM.Functions.ensemble_trainer import EnsembleTrainerSplitDomain, Trainer
...
AllenCahn_instance = AllenCahnEquationDomainDecomposition(data, time_indices=101)
...
result_path = "./logs/result_xpinn_entiredomain_comparison_AC_1d/results" + string_f
p = Path(result_path)
if not p.exists():
...
AllenCahnDeepModel_instance = PDEDeepModelSplitDomain(
                 in_features= 2,
                 out_features=1,
                 hidden_features_str=128, 
                 hidden_layers=3,
...
AllenCahn_no_domain_decompose = AllenCahnEquationEntireDomain(data)
(x_train_list, u_train_list, train_loaders)= AllenCahn_no_domain_decompose.prepare_data_DEIM(
                                                    t_div=3,
                                                    n_basis=1,
                                                    deim_tolerance=1e-6
                                                    )
...
AllenCahnDeepModel_no_domain_decompose = AllenCahnDeepModelEntireDomain(
    in_features=2,
    out_features=1,
    hidden_features_str=128,
    hidden_layers=3,
...
models_list, optim_adam, scheduler = AllenCahnDeepModel_no_domain_decompose.run()
...
string_f_no_domain_decompose = AllenCahn_no_domain_decompose.save_string(
    noise=0,
    learning_rate_inr=1e-5,
    hidden_features_str=128,
    hidden_layers=3,
...
) = AllenCahn_no_domain_decompose.file_path_saving(result_path)
...
for j in range(2):
    if j == 0:
        ...
        axs[j].set_yticks([-0.03, -0.01, 0.03])
    ...
    if j == 1:
        ...
        axs[j].set(ylabel=r"Coefficients $u,\ u^3$")
