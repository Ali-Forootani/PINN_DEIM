...
noise = 0.00  ## percentage of noise considered
...
hidden_features_phi = 64  ## hidden features for the primary SIREN network
...
num_epochs = 2000  ## total number of epoch for training
...
X_train,X_test,u_train,Y_test = train_test_split(X_star, u_star_noisy, test_size=0.99, random_state=42)
...
fig, ax = plt.subplots()
im = ax.scatter(X_train[:,1], X_train[:,0], c=u_train[:,0], marker="x", s=10)
ax.set_xlabel('t')
ax.set_ylabel('x')
...
model_inr = Siren(2, [64, 64, 64, 64], 1).to(device)
