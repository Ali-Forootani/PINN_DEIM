...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 12 07:59:01 2023
...
from PINN_DEIM.Functions.deep_simulation_framework import (
    AllenCahnDeepModel,
    AllenCahnDeepModelEntireDomain,
)
...
from PINN_DEIM.Functions.data_set_preparation import (
    AllenCahnEquation,
    AllenCahnEquationEntireDomain,
)
...
### You have to run `AC_data_generation.py` first then you can run this module
...
filename_coef_track_random_samp = (
    result_path + "/" + "AC_coef_dic_track_list_random_samp" + ".npy"
)


AC_Random_points = np.load(filename_data_random_points, allow_pickle=True)
...
filename_coef_DEIM = (
    result_path + "/" + "AC_coef_dic_list_diff_tol_diff_tim_DEIM" + ".npy"
)
filename_data_t_v_tol_DEIM = (
    result_path + "/" + "AC_list_data_diff_tol_diff_tim_DEIM" + ".npy"
)
filename_coef_track_DEIM = (
    result_path + "/" + "AC_coef_track_dic_list_diff_tol_diff_tim_DEIM" + ".npy"
)


coef_dic_list_DEIM = np.load(filename_coef_DEIM, allow_pickle=True)
list_AC_data_DEIM_t_div_tol = np.load(filename_data_t_v_tol_DEIM, allow_pickle=True)
...
#########################################################
#### Using syles to see how they effect the plots
...
axs[0].set(ylabel=r"\texttt{error} $u_{xx}$")
...
axs[1].set(ylabel=r"\texttt{error} $u,\ -u^3$")
...
fig.legend(
    ...
    ncol=4,
    bbox_to_anchor=(0.52, 1),  # Adjust the coordinates
...
plt.savefig(
    result_path + "/" + "relative_abs_error_" + "GSPINN" + ".png",
    dpi=600,
    bbox_inches="tight",
)
plt.savefig(
    result_path + "/" + "relative_abs_error_" + "GSPINN" + ".pdf",
    dpi=600,
    bbox_inches="tight",
)
...
"""
Computing the average for the random sampling pinn 
"""
...
# plt.plot( np.sort(kmeans_t_div_1.cluster_centers_[:,0],axis=0),
...
axs_2[0].set(ylabel=r"\texttt{error} $u_{xx}$")
...
axs_2[1].set(ylabel=r"\texttt{error} $u,\ -u^3$")
...
rand_1234 = np.vstack((rand_12, rand_34))
rand_12345 = np.vstack((rand_1234, np.array(er_coef_2_t_rand_exp_5)))

row_avg_rand_12345 = np.mean(rand_12345, axis=0)
...
"""
# Assuming you have the data in 'data' (replace with your actual data)
error = [er_coef_2_t_div_1, er_coef_2_t_div_2, er_coef_2_t_div_3, er_coef_2_t_div_4]
num_samples_t_div = [np.array(num_samp_t_div_1),np.array(num_samp_t_div_2),
                     np.array(num_samp_t_div_3),
                     np.array(num_samp_t_div_4),]



plt.plot(num_samples_t_div[3],error[3], num_samples_t_div[2],error[2],
         num_samples_t_div[0],error[0], num_samples_t_div[1],error[1])





num_samples_rand = [np.array(num_samp_t_rand_exp_1),]

error_rand = [er_coef_2_t_rand_exp_1]
...
fig_3, axs_2 = plt.subplots(1, 2, sharey=False, figsize=(12, 5))
#plt.style.use('seaborn')
axs_2[0].bar(np.array(num_samp_t_rand_exp_1), er_coef_1_t_rand_exp_1, alpha=1,
                linewidth=line_width, linestyle="-", color=colors[0])
axs_2[0].bar(np.array(num_samp_t_rand_exp_2), er_coef_1_t_rand_exp_2, alpha=1,
                 linewidth=line_width, linestyle="-", color=colors[0])
axs_2[0].bar(np.array(num_samp_t_rand_exp_3), er_coef_1_t_rand_exp_3, alpha=1,
                linewidth=line_width, linestyle="-",  color=colors[0])
axs_2[0].bar(np.array(num_samp_t_rand_exp_4), er_coef_1_t_rand_exp_4, alpha=1,
                linewidth=line_width, linestyle="-",  color=colors[0])
axs_2[0].bar(np.array(num_samp_t_rand_exp_5), er_coef_1_t_rand_exp_5, alpha=1,
                linewidth=line_width, linestyle="-",  color=colors[0])


axs_2[0].bar(np.array(num_samp_t_div_1), er_coef_1_t_div_1, 
                linewidth=line_width, linestyle="--",  color="k")
axs_2[0].bar(np.array(num_samp_t_div_2), er_coef_1_t_div_2,
                linewidth=line_width, linestyle="--",  color="k")
axs_2[0].bar(np.array(num_samp_t_div_3), er_coef_1_t_div_3,
                linewidth=line_width, linestyle="--",  color="k")
axs_2[0].bar(np.array(num_samp_t_div_4), er_coef_1_t_div_4,
                linewidth=line_width, linestyle="--",  color="k")
...
axs_2[1].bar(np.array(num_samp_t_rand_exp_1), np.log10(er_coef_2_t_rand_exp_1), alpha=1,
                  color=colors[0])
axs_2[1].bar(np.array(num_samp_t_rand_exp_2), np.log10(er_coef_2_t_rand_exp_2), alpha=1,
                  color=colors[0])
axs_2[1].bar(np.array(num_samp_t_rand_exp_3), np.log10(er_coef_2_t_rand_exp_3), alpha=1,
                  color=colors[0])
axs_2[1].bar(np.array(num_samp_t_rand_exp_4), np.log10(er_coef_2_t_rand_exp_4), alpha=1,
                 color=colors[0])
axs_2[1].bar(np.array(num_samp_t_rand_exp_5), np.log10(er_coef_2_t_rand_exp_5), alpha=1,
                  color=colors[0],
                label=r"\texttt{Random} Sampling",
                )

axs_2[1].bar(np.array(num_samp_t_div_1), np.log10(er_coef_2_t_div_1), 
                 width=10 , color= "k")
axs_2[1].bar(np.array(num_samp_t_div_2), np.log10(er_coef_2_t_div_2),
                  color= "k")
axs_2[1].bar(np.array(num_samp_t_div_3), np.log10(er_coef_2_t_div_3),
                  color= "k")
axs_2[1].bar(np.array(num_samp_t_div_4), np.log10(er_coef_2_t_div_4),
                  color="k", 
                label=r"\texttt{Q-DEIM} Sampling",
                )
...
fig_2.legend(
    lines, labels, loc="upper center",  # Change the location to upper center
    ncol=2,
    bbox_transform=fig_2.transFigure,
    fontsize=20,
    frameon=True,
