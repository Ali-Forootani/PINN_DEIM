...
### the loss function Burger

def loss_func_Burger(u, u_pred, f_pred):
...
def loss_func_KDV(u, u_pred, f_pred):
...
def loss_func_CH_2(u, u_pred, f_pred_u):

    loss_data = torch.mean((u - u_pred) ** 2)
    #loss_eq = torch.mean(f_pred ** 2)
    loss_u = torch.mean(f_pred_u ** 2)
    loss = loss_data + loss_u 

    return loss, loss_data, loss_u
...
def equation_residual_u_CH(thetas, time_derivs, coeff_vectors):
    residual = time_derivs - thetas @ coeff_vectors
